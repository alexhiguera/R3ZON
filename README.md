...
### Iteración 14 — *2026-05-02* — Kanban: drag total, creación rápida y persistencia paralela
- **Tarjeta arrastrable desde cualquier parte** ([`src/components/kanban/TaskCard.tsx`](src/components/kanban/TaskCard.tsx)) — eliminado el handle `GripVertical` y aplicados `attributes` + `listeners` del `useSortable` al `<div>` raíz. El click sigue abriendo el modal porque el `PointerSensor` ya tiene `activationConstraint: { distance: 8 }` (sólo se inicia drag tras 8px de movimiento). Cursor `grab/grabbing` en toda la tarjeta y `touch-none`/`select-none` para móvil sin selección de texto accidental.
- **Creación in-line con sólo título** ([`src/components/kanban/InlineTaskAdder.tsx`](src/components/kanban/InlineTaskAdder.tsx)) — nuevo componente que aparece dentro de la columna al pulsar «+». Sólo input de título; **Enter** crea; **Esc** cancela; click fuera con campo vacío también cancela. Tras crear deja el input abierto para añadir varias seguidas. El defaults: prioridad `normal`, sin descripción ni fecha (se editan luego haciendo click en la tarjeta). El botón «Nueva tarea» del toolbar sigue abriendo el modal completo.
- **Estado vacío de columna** ahora es un botón clickeable que también abre el adder in-line («Arrastra aquí o pulsa + para crear»).
- **Persistencia paralela en drag** ([`src/app/(app)/tareas/page.tsx`](src/app/%28app%29/tareas/page.tsx)) — el `for…await` secuencial que persistía las posiciones tras un reorden se sustituyó por `Promise.all(...)`. Reduce el tiempo de bloqueo del cliente HTTP del navegador (antes mantenía N peticiones encoladas seriadas) y baja la probabilidad de race con HMR/navegación. Ambos paths (reorden y cambio de columna) envueltos en `try/catch` que logea sin congelar la UI.

**Sobre el "Failed to fetch" reportado**: el stack trace (`linkClicked → navigateToUnknownRoute → fetchServerResponse`) indica que Next.js intentaba cargar el payload de un nuevo route tras click en un `<Link>` del Sidebar y el fetch falló. Esto NO viene del código del kanban — es un síntoma típico de **chunks stale del dev server tras varios HMR seguidos**. La paralelización del drag mitiga la ventana de race, pero si vuelve a ocurrir: matar `next dev`, borrar `.next/` y reiniciar.
